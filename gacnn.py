...
class GACNN(nn.Module):
    def __init__(self, in_node_nf, hidden_nf, out_node_nf, n_layers=4):
        ...
        for i in range(0, n_layers):
            self.add_module("gcl_%d" % i, nn.Linear(self.hidden_nf,self.hidden_nf))
            self.add_module("relu_%d" % i, nn.ReLU())
    ...
    def forward(self, h, t, edges):
        ...
        for i in range(0, self.n_layers):
            h = self._modules["gcl_%d" % i](h)
            h = self._modules["relu_%d" % i](h)
            # h = torch.cat((h, t), dim=1)
